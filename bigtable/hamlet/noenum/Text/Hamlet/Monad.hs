...
instance Monad m => Monad (Hamlet url m) where
    return x = Hamlet $ \_ _ -> return x
    (Hamlet f) >>= g = Hamlet $ \a b -> f a b >>= \x -> runHamlet (g x) a b
...
instance Monoid HtmlContent where
    ...
    mappend x y = Encoded $ mappend (htmlContentToBS x)
                                    (htmlContentToBS y)
...
outputHtml = output . htmlContentToBS
...
mapH each (Enumerator e) = Hamlet go where
    go surl out = do
        e $ iter' surl out
    iter' surl out val = runHamlet (each val) surl out
...
printHamlet render h = runHamlet h render S.putStr

-- | Returns HTML-ready text (ie, all entities are escaped properly).
htmlContentToBS :: HtmlContent -> ByteString
htmlContentToBS (Encoded t) = t
htmlContentToBS (Unencoded t) = S8.concatMap (pack . encodeHtmlChar) t
